/*********************************************************************************************************************
* LS2K030X Opensource Library 即（LS2K030X 开源库）是一个基于官方 SDK 接口的第三方开源库
...
//              dis_width/dis_height 为屏上显示区域，与源 width/height 不同时按最近邻缩放
//-------------------------------------------------------------------------------------------------------------------
...
{
    uint16 color = 0;
    uint16 dw = dis_width ? dis_width : width;
    uint16 dh = dis_height ? dis_height : height;

    if (width == 0 || height == 0 || dw == 0 || dh == 0)
        return;

    for (uint16 dy = 0; dy < dh; dy++)
    {
        for (uint16 dx = 0; dx < dw; dx++)
        {
            uint16 src_x = (uint16)(((uint32_t)dx * (uint32_t)width) / (uint32_t)dw);
            uint16 src_y = (uint16)(((uint32_t)dy * (uint32_t)height) / (uint32_t)dh);
            uint8 gray_value = image[(uint32_t)src_y * width + src_x];
            if(threshold == 0)
            {
            ...
            draw_point((uint16)(x + dx), (uint16)(y + dy), color);
        }
    }
}
...
void zf_device_tft180::show_rgb565_image (uint16 x, uint16 y, const uint16 *image, uint16 width, uint16 height, uint16 dis_width, uint16 dis_height, uint8 color_mode)
{
    ...
    for (uint16 dy = 0; dy < dh; dy++)
    {
        for (uint16 dx = 0; dx < dw; dx++)
        {
            uint16 src_x = (uint16)(((uint32_t)dx * (uint32_t)width) / (uint32_t)dw);
            uint16 src_y = (uint16)(((uint32_t)dy * (uint32_t)height) / (uint32_t)dh);
            uint16 color = image[(uint32_t)src_y * width + src_x];
            if(1 == color_mode)
            {
