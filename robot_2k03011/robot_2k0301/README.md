...
```bash
./scripts/deploy_ls2k0301.sh root@BOARD_IP
```

程序会部署到 `/root/robot_2k0301`。登录板子后先执行只读预检；它检查架构、文件、动态库、
设备节点和 TCP 端口，不会驱动电机：

```bash
cd /root/robot_2k0301
./scripts/board_preflight.sh
```

默认使用 `config/robot_bringup.yaml`，其中远程速度限制为 `0.15 m/s`，雷达停车/减速距离为
`500/800 mm`。原有标定参数保留在 `config/robot.yaml`。确认设备节点后，架空驱动轮并执行
无非零电机输出的诊断：

```bash
./scripts/board_diagnostics.sh
```

诊断依次检查板端算法、原始编码器、雷达和摄像头。任何设备节点或动态库缺失都应先解决，
不要直接启动自动任务。
...
```bash
./scripts/check_board_connection.sh 192.168.43.192
```

电脑必须能路由到板子所在热点网段。特别是 VMware 使用 NAT 且虚拟机地址不是
`192.168.43.x` 时，应先把虚拟网卡桥接到连接手机热点的物理网卡，再判断板端故障。

如果电脑网络正确但板子仍未取得 IP，通过串口进入板子后运行只读诊断：

```bash
cd /root/robot_2k0301
./scripts/board_network_diagnose.sh
```

确认 `/etc/wpa_supplicant.conf` 中 SSID/密码正确后，可安装不会阻塞开机、失败后自动重试的
Buildroot 启动脚本：

```bash
./scripts/install_board_autonetwork.sh
reboot
```

安装器会备份并禁用旧的 `S99wifi_start`/`S96_wifi_start`，使用
`/etc/init.d/S96_robot_network` 后台连接 Wi‑Fi 和获取 DHCP 地址，日志保存在
`/var/log/robot-network.log`。不要在未确认热点配置时直接安装。
...
`build_ls2k0301.sh` 会同时交叉编译 `robot_board_vision`，并把本地
HOG+SVM 模型复制到可执行文件旁的 `models/`。部署时将程序和模型目录一起复制到板子：

```bash
scp -r build/ls2k0301/board_app/robot_board_vision \
  build/ls2k0301/board_app/models root@192.168.43.192:/root/robot_2k0301/
```

直接读取板端 UVC 摄像头进行遗失物扫描：

```bash
cd /root/robot_2k0301
./robot_board_vision --mode lost-item --camera /dev/video0 \
  --model models/lost_item_hog_svm.xml \
  --metadata models/lost_item_hog_svm.json \
  --output /tmp/lost_item_detection.jpg
```

程序逐帧输出紧凑 JSON，并把最新画框结果写入 `--output`。`lost-item-crop` 模式可对
已经裁好的候选图进行模型校验。连续帧检测会分配跟踪 ID，连续 3 帧命中后才确认，
避免单帧噪声直接产生遗失物告警。

交叉工具链中的 OpenCV 4.10 使用 `objdetect` 内的官方 `ArucoDetector`；工程同时保留了
兼容旧版 OpenCV 的轻量 `DICT_5X5_250` 解码器。两条路径都支持本项目分配的书籍标签
ID `101、201、202、203、204、205`：

```bash
./robot_board_vision --mode book --camera /dev/video0 \
  --expected-id 203 --output /tmp/book_detection.jpg
```

识别器支持标签旋转、透视校正和最多 3 bit 汉明误码，并保留书名映射、书架固定顺序、
目标书标红、JSON 结果和标注图输出。

需要在电脑或手机查看板端识别画面时，可由同一视觉进程输出带标注的 JPEG 帧流；帧格式为
4 字节网络序长度加 JPEG 数据，慢客户端不会阻塞识别线程：

```bash
./robot_board_vision --mode book --camera /dev/video0 --expected-id 203 \
  --stream-port 5000 --jpeg-quality 80 --output ''
```

这里识别、跟踪和画框都在板端执行，电脑只接收结果/画面。端口尚未在真实板卡网络上验证。

## 板端语音、烟雾和任务服务

以下原来适合由电脑节点承担、但不依赖 ROS2 的功能已实现为板端 C++ 服务：

- CI1302 串口命令解析（支持粘包、拆包）及语音播报帧发送
- MQ-2 ADC sysfs 采样、连续 3 次确认和告警/解除双阈值滞回
- 找书、书架巡检、遗失物巡检、安全巡检和返航任务步骤编排
- 固定容量事件缓存、JSONL 本地日志和 TCP 状态快照
- 板端子进程执行器：按任务顺序启动导航和视觉，并将退出结果回送任务状态机
- 取消、烟雾告警和服务退出时终止当前动作，底盘子进程执行停车清理
...
默认以安全模式运行。安全模式接收语音/网络任务并显示待执行步骤，但不会启动导航或视觉子进程：

```bash
./scripts/start_board_runtime.sh
```
...
不依赖 CI1302 也可以从电脑发送任务或停车命令：

```bash
./scripts/send_board_command.sh BOARD_IP FIND_BOOK
./scripts/send_board_command.sh BOARD_IP CANCEL
./scripts/send_board_command.sh BOARD_IP E_STOP
```

解锁模式默认拒绝网络发起新任务，只允许无条件的 `CANCEL/E_STOP`。确实需要电脑远程发起任务时，
在板端和电脑端设置相同的临时令牌后再启动/发送，例如
`export ROBOT_COMMAND_TOKEN='本次调试令牌'`；CI1302 本地语音命令不受该令牌影响。

只有编码器方向、舵机方向、雷达距离和摄像头都通过单项测试后，才允许启动闭环执行：

```bash
# 仍需架空驱动轮并准备物理急停
./scripts/start_board_runtime.sh --arm
```

`--arm` 会启用 `--execute-actions`：导航成功后自动启动对应视觉识别，识别程序只有在限定帧数内
确认目标才返回成功，结果会继续推进任务。导航进程成功后暂以目标点作为下一段起点；由于尚无
板端 AMCL，实际打滑会造成累计误差。网络 `E_STOP` 是辅助措施，不能替代物理断电急停。
...
当前导航按需求关闭了雷达动态避障：雷达观测不会写入规划地图、不会触发近障停/减速，
也不会因路径被占用而重规划。规划器仍遵守静态地图中的墙体和边界；实时雷达、AMCL
位姿协方差和里程计仍用于定位质量门控，定位丢失时电机保持停止。规划失败、偏离路径
或 3 秒没有运动进展时最多自动恢复 3 次，之后才将任务标记为失败。
...
```bash
NAV_RECORD_BAG=1 BOARD_IP=192.168.43.192 ./scripts/start_navigation.sh
```
...
以下 ROS 无关算法已经提供板端 C++ 实现：

- 编码器与 IMU 里程计积分
- PGM/YAML 地图加载和动态障碍膨胀
- Hybrid A* 阿克曼全局规划
- 曲率前馈、前视跟踪、横向误差和航向 PID
- 加速度、减速度和加加速度限制
- 定位质量门控、路径阻塞、偏航重规划
- 任务点队列与前沿目标选择
- 雷达断流、近障减速和停车
...
./robot_board_navigation --plan maps/library.yaml 0 0 0 0.5 0 0
```

`robot_board_app` 还提供直接控制板端硬件的实验入口。所有坐标单位为米，yaw 单位为弧度：

```bash
./robot_board_app --board-navigation \
  --map maps/library.yaml \
  --start-x 0 --start-y 0 --start-yaw 0 \
  --goal-x 0.5 --goal-y 0 --goal-yaw 0 \
  --timeout 120
```

该模式由同一进程独占雷达、编码器、IMU、电机和舵机，不能同时启动
`lidar-stream` 或 `mapping-drive`。当前板端没有 AMCL，起点依赖人工输入，运行中使用
编码器与 IMU 航位推算，因此长距离会累积漂移。这是已交叉编译但尚未在实车验证的入口；
第一次运行必须架空驱动轮并准备物理急停，确认转向方向、编码器符号、IMU 符号和地图坐标
无误后才能落地低速测试。

AMCL、slam_toolbox、ROS2 TF/topic、RViz、Web 上位机和报告服务仍运行在电脑端；这些程序
依赖完整 ROS2 运行时或图形/后台环境，不属于可直接移入当前板端控制进程的算法。
...
8. 导航定位方案 A / B
   ...
   ```bash
   NAV_LOCALIZER=amcl BOARD_IP=192.168.43.192 ./scripts/start_navigation.sh
   ```
...
BOARD_IP=192.168.43.192 ./scripts/start_mapping_keyboard.sh
```
...
当前场地墙内净尺寸固定为 `1.8 m x 1.8 m`。如显式设置
`SKIP_MAP_QUALITY_CHECK=0`，保存脚本会从最大连通空闲区
...
scp build/ls2k0301/board_app/robot_board_app \
  root@192.168.43.192:/home/root/robot_2k0301/robot_board_app.new
ssh root@192.168.43.192
cd /home/root/robot_2k0301
...
```bash
scp config/robot.yaml root@192.168.43.192:/home/root/robot_2k0301/config/robot.yaml
```
